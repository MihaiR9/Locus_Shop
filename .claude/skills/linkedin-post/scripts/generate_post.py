#!/usr/bin/env python3
"""Generate a Finbot LinkedIn post (HTML + optional PNG).
...
Usage examples:
  python generate_post.py --variant statement --theme bone \\
      ...
      --out ./out/01-banii-tai-live

  python generate_post.py --variant number --theme ink \\
      --number "41" --unit "ore" \\
      --headline "pierdute lunar pe contabilitatea manuală." \\
      --eyebrow "Cifră · Teaser 02" \\
      --source "Estimare Finbot, 2026" \\
      --out ./out/02-41-ore --render-png
"""
...
FORMATS = {
    "4x5": (1080, 1350),  # portrait, max screen real estate on feed
    "1x1": (1080, 1080),  # classic square
    "16x9": (1200, 675),  # link preview / banner
}
...
def auto_size_headline(text: str, format_key: str) -> int:
    ...
    The default is sized for ~30 characters. We bump down progressively for
    longer headlines so they don't bust the canvas. Tuned by eye against the
    4:5 canvas; other formats inherit similar ratios."""
    chars = len(text)
    if format_key == "4x5":
    ...
    if format_key == "1x1":
        ...
        return 68
    # 16x9
    if chars <= 18: return 120
    if chars <= 30: return 100
    if chars <= 48: return 84
    return 68
...
def auto_size_question(text: str, format_key: str) -> int:
    ...
    if format_key == "1x1":
        ...
        return 50
    if chars <= 40: return 80
    if chars <= 70: return 64
    return 54
...
def linebreak_to_html(text: str) -> str:
    ...
    Accept both real newlines and the two-char literal "\\n" (the latter is
    convenient when the value comes from a shell arg, where "\n" is not
    expanded by default)."""
    text = text.replace("\\n", "\n")
...
def block_or_empty(condition: bool, html: str) -> str:
...
class Content:
    ...
    subtext: str = ""
    meta: str = "TEASER"
    cta: str = "Înscrie-te pe lista de așteptare"
...
def render_html(variant: str, theme: str, format_key: str, content: Content) -> str:
    ...
    grid_class = "has-grid" if theme_vars["use_grid"] else ""
...
def render_png(html_path: Path, png_path: Path, width: int, height: int) -> None:
    ...
    if not chrome:
        print("WARN: Chrome not found, skipping PNG render. "
              ...
              file=sys.stderr)
        return
    # 2x scale factor for Retina-quality output
    cmd = [
        chrome,
        "--headless=new",
        "--disable-gpu",
        ...
        "--virtual-time-budget=4000",  # let webfonts load
...
def main() -> int:
    ...
    p.add_argument("--meta", default="TEASER",
                   help="Mono text top-right (e.g. '01 / TEASER')")
    ...
    # CTA
    p.add_argument("--cta", default="Înscrie-te pe lista de așteptare")
